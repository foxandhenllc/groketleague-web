...
function stepBall(ball, dt) {
  ...
  const limX = FW / 2 + 1.6;
  const limZ = FL / 2 + 2.6;
  if (ball.x > limX) { ball.x = limX; ball.vx *= -0.55; }
  if (ball.x < -limX) { ball.x = -limX; ball.vx *= -0.55; }
  const inMouth = Math.abs(ball.x) < GOAL_W / 2 - 0.1 && ball.y < GOAL_H - 0.2;
  if (ball.z > limZ) {
    if (inMouth && ball.z > FL / 2 + 0.4) return "B";
    ball.z = limZ;
    ball.vz *= -0.55;
  }
  if (ball.z < -limZ) {
    if (inMouth && ball.z < -FL / 2 - 0.4) return "A";
    ball.z = -limZ;
    ball.vz *= -0.55;
  }
  return null;
