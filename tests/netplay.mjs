...
try {
  ...
  const authority = h.role === 'host' ? host : second;
  const remote = h.role === 'guest' ? host : second;
  await remote.locator('#menuBtn').click();
  assert.equal((await state(remote)).menuOpen, true);
  await remote.locator('#fsdToggle').click();
  await until(authority, () => { const s = JSON.parse(render_game_to_text()); return s.peerFsd && s.B._ai.t > 0; });
  assert.equal((await state(authority)).fsd, false, 'Guest toggles only its own car');
  await remote.locator('#fsdToggle').click();
  await until(authority, () => !JSON.parse(render_game_to_text()).peerFsd);
  const bAi = (await state(authority)).B._ai.t;
  await authority.waitForTimeout(200);
  assert.equal((await state(authority)).B._ai.t, bAi, 'Guest AI stops when FSD is disabled');
  await remote.locator('#resumeBtn').click();
  await authority.locator('#menuBtn').click();
  await authority.locator('#fsdToggle').click();
  await until(remote, () => { const s = JSON.parse(render_game_to_text()); return s.peerFsd && s.P._ai.t > 0; });
  assert.equal((await state(remote)).fsd, false, 'Host toggles only its own car');
  await authority.locator('#fsdToggle').click();
  await until(remote, () => !JSON.parse(render_game_to_text()).peerFsd);
  const pAi = (await state(authority)).P._ai.t;
  await authority.waitForTimeout(200);
  assert.equal((await state(authority)).P._ai.t, pAi, 'Host AI stops when FSD is disabled');
  await authority.locator('#resumeBtn').click();
  console.log('PASS independent host/guest FSD enable and disable, authoritative AI, live menu');
  await second.close();
  await until(host, () => JSON.parse(window.render_game_to_text()).mode === 'garage');
  ...
  await host.locator('#menuBtn').click(); assert.equal((await state(host)).paused, true);
  const pausedTime = (await state(host)).timeLeft;
  await host.locator('#fsdToggle').click();
  assert.equal((await state(host)).fsd, true);
  assert.equal((await state(host)).timeLeft, pausedTime, 'FSD toggle does not reset or resume the match');
  await host.locator('#resumeBtn').click();
  await until(host, () => JSON.parse(render_game_to_text()).P._ai.t > 0);
  await host.locator('#menuBtn').click(); await host.locator('#fsdToggle').click();
  const aiBefore = (await state(host)).P._ai.t;
  await host.locator('#resumeBtn').click(); await host.waitForTimeout(200);
  assert.equal((await state(host)).P._ai.t, aiBefore);
  await host.locator('#menuBtn').click();
  await host.locator('#newGameBtn').click();
  assert.equal(await host.locator('#menuBtn').isVisible(), false);
  console.log('PASS offline mid-match FSD enable/disable without resetting timer or match');
  ...
  await mobile.locator('#menuBtn').tap();
  assert.equal((await state(mobile)).paused, true);
  await mobile.locator('#fsdToggle').tap();
  assert.equal(await mobile.locator('#fsdToggle').getAttribute('aria-pressed'), 'true');
  await mobile.screenshot({ path: `${out}/mobile-menu-fsd.png` });
  await mobile.locator('#resumeBtn').tap();
  await until(mobile, () => JSON.parse(render_game_to_text()).P._ai.t > 0);
  await mobile.locator('#menuBtn').tap(); await mobile.locator('#fsdToggle').tap();
  assert.equal((await state(mobile)).fsd, false);
  await mobile.locator('#newGameBtn').tap();
  assert.equal((await state(mobile)).mode, 'garage');
  console.log('PASS touch-only menu, FSD toggle, resume and return to garage');
